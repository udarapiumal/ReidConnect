...
public class GlobalExceptionHandler {
        ...
        /**
         * Handle database constraint violations (duplicate username, email, club name)
         */
        @ExceptionHandler(DataIntegrityViolationException.class)
        public ResponseEntity<ErrorResponse> handleDataIntegrityViolationException(
                        DataIntegrityViolationException ex, WebRequest request) {
                String message = "A record with the provided information already exists.";
                String rootMessage = ex.getMostSpecificCause().getMessage();

                if (rootMessage != null) {
                        String lower = rootMessage.toLowerCase();
                        if (lower.contains("key (username)")) {
                                message = "This username is already taken. Please choose a different one.";
                        } else if (lower.contains("key (email)")) {
                                message = "An account with this email already exists.";
                        } else if (lower.contains("key (club_name)")) {
